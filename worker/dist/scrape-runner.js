...
async function runScrape(log) {
    ...
    let totalScraped = 0;
    const apifyClient = new apify_client_1.ApifyClient({ token: process.env.APIFY_API_TOKEN });
        ...
        for (const businessType of config.businessTypes) {
            ...
            for (const city of cities) {
                try {
                    ...
                    log('info', `    Found ${items.length} results, ${items.filter((i) => i.phone).length} with phone`);
                }
                catch (error) {
